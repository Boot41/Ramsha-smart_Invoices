    ...
    try:
        if message_type == 'human_input_submission':
            ...
            try:
                ...
                # Clear the waiting flags and reset workflow completion status
                updated_state["awaiting_human_input_websocket"] = False
                updated_state["awaiting_websocket_connection"] = False
                
                # Reset workflow completion to allow continuation
                if updated_state.get("workflow_completed"):
                    updated_state["workflow_completed"] = False
                    logger.info("🔄 Reset workflow_completed flag to allow continuation after human input")
...
async def get_workflow_ui_template(
    ...
    """
    🎨 Get the generated professional invoice UI template from a completed workflow
    
    Returns the HTML/CSS invoice template generated by the UI Invoice Generator agent.
    Only available for completed workflows.
    """
    logger.info(f"🎨 API: Getting UI template - ID: {workflow_id}, User: {current_user['user_id']}")
