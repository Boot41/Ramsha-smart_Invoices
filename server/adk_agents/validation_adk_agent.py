...
class ValidationADKAgent(BaseADKAgent):
    ...
    _natural_language_service = PrivateAttr()
    ...
    def __init__(self):
        ...
        self._natural_language_service = get_natural_language_correction_service()
    ...
    async def _process_natural_language_query(self, state: Dict[str, Any], query: str) -> Dict[str, Any]:
        """Process natural language query to extract field corrections"""
        try:
            ...
            if current_unified_data:
                ...
                # Create minimal invoice data for extraction
                from schemas.unified_invoice_schemas import UnifiedInvoiceMetadata
            ...
            # Get missing fields from validation request
            human_input_request = state.get("human_input_request", {})
            missing_fields = human_input_request.get("missing_required_fields", [])
            validation_issues = [issue.get("description", "") for issue in human_input_request.get("validation_issues", [])]
            
            # Process the natural language query
            result = await self._natural_language_service.process_natural_language_query(
                query=query,
                current_invoice_data=unified_invoice,
                missing_fields=missing_fields,
                validation_issues=validation_issues
            )
            
            if result.get("success"):
                self.logger.info(f"✅ Extracted {len(result.get('corrections', {}))} fields from natural language query")
                return result.get("corrections", {})
            else:
                self.logger.warning(f"⚠️ Natural language extraction failed: {result.get('error')}")
                return {}
    ...
    async def handle_human_input_response(self, state: Dict[str, Any], context: InvocationContext, human_input_data: Dict[str, Any]) -> AsyncGenerator[SimpleEvent, None]:
        ...
        try:
            # Ensure human_input_data is a dictionary
            if isinstance(human_input_data, str):
                # If it's a string, treat it as a natural language query
                natural_language_query = human_input_data
                human_input_data = {"query": natural_language_query} # Wrap it in a dict
            else:
                natural_language_query = human_input_data.get('query') or human_input_data.get('message')
            
            if natural_language_query:
                # Process natural language query
                yield self.create_progress_event(f"🗣️ Processing natural language query: '{natural_language_query[:50]}...'", 20.0)
                field_values = await self._process_natural_language_query(
                    state, natural_language_query
                )
                yield self.create_progress_event(f"✅ Extracted {len(field_values)} fields from natural language", 30.0)
            else:
                # Process structured field values (legacy approach)
                field_values = human_input_data.get('field_values', {}) # Default to empty dict if 'field_values' not present
                yield self.create_progress_event(f"📝 Processing structured field values: {field_values}", 20.0)
            ...
            # Convert any enum values in field_values to their string representation
            ...
            corrected_unified_data = unified_invoice.apply_manual_corrections(processed_field_values)
